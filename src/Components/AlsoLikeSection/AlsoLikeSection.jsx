...
function AlsoLikeSection({ item }) {
  const productId = item.id;

  const url = `https://asos10.p.rapidapi.com/api/v1/getYouMightAlsoLike?productId=${productId}&currency=USD&country=US&store=US&language=en-US`;
  const options = {
    method: "GET",
    headers: {
    ...
    <Column $alignItems={"flex-start"}>
      ...
      >
        {items.slice(0, maxItemsToShow).map((item) => (
          <Item key={item.id} item={item} height={"auto"} width={"180px"} />
        ))}
      </Content>
    </Column>
  );
